...
func (s *XLSuite) TestClusterInHandler(c *C) {
	...
	c.Assert(clusterOp2tag(UpaxClusterMsg_Query), Equals, uint(2))
	c.Assert(clusterOp2tag(UpaxClusterMsg_Get), Equals, uint(3))
	c.Assert(clusterOp2tag(UpaxClusterMsg_IHave), Equals, uint(4))
	c.Assert(clusterOp2tag(UpaxClusterMsg_Put), Equals, uint(5))
	c.Assert(clusterOp2tag(UpaxClusterMsg_Bye), Equals, uint(6))
